...
public class DeviceSessionCommandService {
    ...
    public List<DeviceSessionViewDTO> listUserSessions(String codeUser, String currentDeviceId, String requestIp, String requestLocation) {
        ...
        List<DeviceSessionViewDTO> result = new ArrayList<>();
        ...
        try {
            ...
            if (!devices.isEmpty()) {
                for (UserDevice dev : devices) {
                    ...
                    result.add(new DeviceSessionViewDTO(
        ...
        return activeSessions.stream().map(s -> {
            ...
            return new DeviceSessionViewDTO(
                    s.getSessionId(),
                    s.getDeviceId(),
                    s.getDeviceName(),
                    s.getDeviceType(),
    ...
    public List<TenantDeviceBlacklistViewDTO> listBlacklistForUser(UUID companyId, String actorCodeUser, String targetCodeUser) {
        ...
        return listBlacklist(target.getCodeUser().toString()).stream()
                .map(entry -> TenantDeviceBlacklistViewDTO.from(entry, writable))
                .toList();
    }
    ...
    public org.springframework.data.domain.Page<TenantDeviceBlacklistViewDTO> searchTenantBlacklist(
        ...
        return deviceBlacklistRepository.search(companyId, filterCodeUser, deviceId, deviceName, ipAddress, from, to, pageable)
                .map(entry -> TenantDeviceBlacklistViewDTO.from(entry, writableFor(actor, entry.getCodeUser(), companyId, targetCache)));
    }

    public org.springframework.data.domain.Page<DeviceSessionViewDTO> searchTenantSessions(
    ...
    private DeviceSessionViewDTO toTenantSessionView(User actor, UserDevice device, UUID companyId, Map<String, User> targetCache) {
    ...
    private DeviceSessionViewDTO withTenantFields(DeviceSessionViewDTO view, String codeUser, boolean writable) {
        return new DeviceSessionViewDTO(
                view.sessionId(),
                view.deviceId(),
                view.deviceName(),
